...
const user = socket.user
    ...
    socket.on("START_TYPING", (members) => {
        const chatId = members.chatId
        const memberSockets = getSockets(members.members)
        socket.to(memberSockets).emit("START_TYPING", { chatId })
    })
    socket.on("STOP_TYPING", (members) => {
        const chatId = members.chatId
        const memberSockets = getSockets(members.members)
...
server.listen(port, () => {
    console.log(`server starts on port ${port} in ${envMode} mode`);
})
